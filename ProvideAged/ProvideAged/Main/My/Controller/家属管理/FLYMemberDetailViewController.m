//
//  FLYMemberDetailViewController.m
//  ProvideAged
//
...
@interface FLYMemberDetailViewController () < UITextFieldDelegate, UICollectionViewDataSource, UICollectionViewDelegate >
...
@implementation FLYMemberDetailViewController
...
{
    NSDictionary * params = @{ @"id" : self.memberModel.idField };
    
    [FLYNetworkTool postRawWithPath:API_AUTHORIZELOCK params:params loadingType:(FLYNetworkLoadingTypeNone) loadingTitle:nil isHandle:YES success:^(id  _Nonnull json) {
...
- (void)submitNetwork
{
    ...
    NSDictionary * params = @{ @"name" : self.nameTF.text, @"phone" : self.phoneNumTF.text, @"relationship" : self.relationshipTF.text, @"oldManInfoId" : self.memberModel.oldManInfoId, @"id" : self.memberModel.idField, @"familyAuthArgList" : familyAuthArgList };
    
    [FLYNetworkTool postRawWithPath:API_MEMBERUPDATE params:params success:^(id json) {
...
- (void)deleteNetwork
{
    NSDictionary * params = @{ @"ids" : @[self.memberModel.idField] };
    
    [FLYNetworkTool postRawWithPath:API_MEMBERDELETE params:params success:^(id json) {
...
- (void)initUI
{
    self.navigationItem.title = @"家属详情";
...
- (void)addClick:(UIButton *)button
{
    ...
    UIAlertController * alertController = [UIAlertController alertControllerWithTitle:@"提示" message:@"是否确定修改该家属" preferredStyle:(UIAlertControllerStyleAlert) titles:@[@"取消", @"确定"] alertAction:^(NSInteger index) {
...
{
    UIAlertController * alertController = [UIAlertController alertControllerWithTitle:@"提示" message:@"是否确定删除该家属" preferredStyle:(UIAlertControllerStyleAlert) titles:@[@"取消", @"确定"] alertAction:^(NSInteger index) {
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    vc.memberIdField = self.memberModel.idField;
...
- (UITextField *)phoneNumTF
{
    if( _phoneNumTF == nil )
    {
        ...
        _phoneNumTF.text = self.memberModel.phone;
    }
    return _phoneNumTF;
}
...
- (UITextField *)nameTF
{
    if( _nameTF == nil )
    {
        ...
        _nameTF.text = self.memberModel.name;
    }
    return _nameTF;
}
...
- (UITextField *)relationshipTF
{
    if( _relationshipTF == nil )
    {
        ...
        _relationshipTF.text = self.memberModel.relationship;
    }
...
-(FLYButton *)deleteBtn
{
    if ( _deleteBtn == nil )
    {
        _deleteBtn = [FLYButton buttonWithTitle:@"删除" titleColor:COLORHEX(@"#2BB9A0") font:FONT_M(16)];
